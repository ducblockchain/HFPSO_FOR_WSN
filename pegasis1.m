...
avg_node(i)=temp2;
    ...
    ylabel 'Energy ( J )';
    hold on;
    % Plotting Simulation Results "Total Energy consumed by All Nodes per Transmission" %
figure(5)
plot(1:rnd-1,total_energy(1:rnd-1),'-b','Linewidth',2);
A = table((1:rnd-1)', total_energy(1:rnd-1)');
writetable(A, 'Pegasis_total_energy.txt')
title ({'PEGASIS'; 'Total Energy consumed by All Nodes per Transmission';})
xlabel 'Transmissions';
...
writetable(B, 'Pegasis_remain_energy.txt')
title({'LEACH'; 'Total Remaining Energy of All Active Nodes per Round'});
xlabel('Rounds');
ylabel('Total Remaining Energy ( J )');
hold on;
figure(8)
plot(1:rnd,energy_per_round(1:rnd),'-b','Linewidth',2);
C = table((1:rnd)', energy_per_round(1:rnd)');
writetable(C, 'Pegasis_energy_per_round.txt')
