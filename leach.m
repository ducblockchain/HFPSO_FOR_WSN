   ...
   if (SN(i).cond == 1) && (SN(i).role == 0) && (CLheads > 0)
   	if SN(i).E > 0
        ...
        if SN(SN(i).chid).E > 0 && SN(SN(i).chid).cond == 1 && SN(SN(i).chid).role == 1
            ...
            if SN(SN(i).chid).E <= 0  % if cluster head's energy depletes with reception
                SN(SN(i).chid).cond = 0;
    ...
    if SN(i).E <= 0       % if node's energy depletes with transmission
        dead_nodes = dead_nodes + 1;
        operating_nodes = operating_nodes - 1;
        SN(i).cond = 0;
        SN(i).chid = 0;
...
for i = 1:n
    if (SN(i).cond == 1) && (SN(i).role == 1)
        ...
        if SN(i).E <= 0     % if cluster head's energy depletes with transmission
            dead_nodes = dead_nodes + 1;
            operating_nodes = operating_nodes - 1;
            SN(i).cond = 0;
            SN(i).rop = rnd;
...
writetable(A, 'Leach_total_energy.txt')
...
writetable(B, 'Leach_remain_energy.txt')
...
writetable(C, 'Leach_energy_per_round.txt')
